<template>
  <div>
      ...
      <div class="col-md-6">
        <div class="panel panel-primary">
          <div class="panel-heading">
            ...
            {{ conversation.group_conversation.name }}
            <button
              class="btn btn-danger btn-sm pull-right"
              @click="leaveFromGroup"
              type="button"
            >Leave</button>
          </div>
          <div class="panel-body">
            <ul class="chat" v-chat-scroll>
              ...
              >
                ...
                >
                  <img
                    :src="'http://placehold.it/50/FA6F57/fff&text='+ message.sender.name"
                    alt="User Avatar"
                    class="img-circle"
                  />
                </span>
                <div class="chat-body clearfix">
                  <div class="header">
                    <small class="text-muted">
                      ...
                      <timeago :since="message.created_at" :auto-update="10"></timeago>
                    </small>
                    <strong
                      v-bind:class="{ 'pull-right' : check(message.sender.id) , 'pull-left' : !check(message.sender.id)}"
                      class="primary-font"
                    >{{ message.sender.name }}</strong>
                  </div>
                  <p
                    v-bind:class="{ 'pull-right' : check(message.sender.id) , 'pull-left' : !check(message.sender.id)}"
                  >{{ message.text }}</p>
                  <div class="row">
                    <div class="col-md-3" v-for="file in message.files" v-bind:key="file">
    ...
    <div class="row">
      <div class="col-md-3" v-for="file in conversation.files" v-bind:key="file">
        <img :src="file.file_details.webPath" alt class="img-responsive" />
...
export default {
  ...
  methods: {
    ...
    leaveFromGroup() {
      axios
        .post("/group/chat/leave/" + this.groupConversationId)
        .then(response => {
          window.location = "/";
        });
    },
    listenForNewMessage: function() {
      Echo.join(this.channel)
        ...
        .listen(
          "\\PhpJunior\\LaravelVideoChat\\Events\\NewGroupConversationMessage",
          data => {
            var self = this;
